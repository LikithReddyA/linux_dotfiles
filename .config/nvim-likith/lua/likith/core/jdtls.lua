...
--   -- local jdtls_path = jdtls.get_install_path and jdtls:get_install_path() or jdtls.path or jdtls:path()
--   local jdtls_path = "/home/likith/.local/share/nvim-likith/mason/packages/jdtls"
...
--   local lombok = "/home/likith/.local/share/nvim-likith/mason/packages/jdtls/lombok.jar"
...
--   -- local java_debug_path = java_debug:path()
--   local java_debug_path = "/home/likith/.local/share/nvim-likith/mason/packages/java-debug-adapter"
--
--   local bundles = {
...
--   -- local java_test_path = java_test:path()
--   local java_test_path = "/home/likith/.local/share/nvim-likith/mason/packages/java-test"
...
-- File: lua/likith/core/jdtls.lua
-- Neovim JDTLS configuration for Java LSP
--
local M = {}
...
-- Check if JDTLS is already attached for this project root
local function is_jdtls_attached(root_dir)
...
-- Get paths to JDTLS, config, and Lombok jar
local function get_jdtls()
  local jdtls_path = "/home/likith/.local/share/nvim-likith/mason/packages/jdtls"
  local launcher = vim.fn.glob(jdtls_path .. "/plugins/org.eclipse.equinox.launcher_*.jar")
  local SYSTEM = "linux"
...
-- Get bundles for debug and testing
local function get_bundles()
  local bundles = {}
...
-- Get workspace directory for the project
local function get_workspace()
  local home = os.getenv("HOME")
  local workspace_path = home .. "/code/workspace/"
  local project_name = vim.fn.fnamemodify(vim.fn.getcwd(), ":p:h:t")
...
local function java_keymaps()
  ...
  vim.keymap.set("v", "<leader>Jt", "<Esc><Cmd>lua require('jdtls').test_nearest_method(true)<CR>", opts)
...
function M.setup_jdtls()
  ...
  local on_attach = function(client, bufnr)
    ...
    local bundle = get_bundles()

    dap.setup_dap({ hotcodereplace = "auto" }) -- setup debug adapter
    dap.setup_dap_main_class_configs({ bundles = bundle })
  end
...
-- Safe wrapper to avoid duplicates
function M.setup_jdtls_safe()
  local jdtls = require("jdtls")
  local root_dir = jdtls.setup.find_root({ ".git", "mvnw", "gradlew", "pom.xml", "build.gradle" })
  if root_dir and not is_jdtls_attached(root_dir) then
    M.setup_jdtls()
  end
end
